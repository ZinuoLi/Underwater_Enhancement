...
class FFCResNet(nn.Module):
    ...
    def __init__(self, block, layers, zero_init_residual=False, groups=1, width_per_group=64, norm_layer=None,
                 ratio=0.5, lfu=True, use_se=False, out_h=64, out_w=64):
        ...
        self.lfu = lfu
        self.use_se = use_se
        self.conv1 = nn.Conv2d(9, inplanes, kernel_size=7,
        ...
        self.avgpool = nn.AdaptiveAvgPool2d((out_h, out_w))
        # self.fc = nn.Linear(inplanes * 8 * block.expansion, num_classes)
        self.conv_out = nn.Conv2d(inplanes * 8 * block.expansion, 9, kernel_size=1, stride=1, padding=0, bias=True)
...
if __name__ == '__main__':
    tensor = torch.randn(1, 9, 64, 64).cuda()
    print(tensor.shape)
    model = FFCResNet(BasicBlock, [2, 2, 2, 2], out_h=64, out_w=64)
    model = model.cuda()
    # 57200896
