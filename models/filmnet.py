...
class TransLow(nn.Module):
    def __init__(self, num_residual_blocks=5):
        ...
        blocks = [nn.Conv2d(3, 16, 3, padding=1),
        ...
        blocks += [nn.Conv2d(64, 16, 3, padding=1),
                   nn.LeakyReLU(),
...
class TransHigh(nn.Module):
    ...
    def forward(self, x, pyr_original, fake_low):
        ...
        # mask = nn.functional.interpolate(mask, size=(pyr_original[-4].shape[2], pyr_original[-4].shape[3]))
        # result_highfreq = torch.mul(pyr_original[-4], mask) + pyr_original[-4]
        ...
        # pyr_result.append(agg3_rgb)

        # mask = nn.functional.interpolate(mask, size=(pyr_original[-5].shape[2], pyr_original[-5].shape[3]))
        # result_highfreq = torch.mul(pyr_original[-5], mask) + pyr_original[-5]
